...
public class cameraMoveToChild : MonoBehaviour {
	...
	void Update () {
        ...
        if (moveToTarget)
        {
            float step = speed * Vector3.Distance(Camera.main.transform.position, childPosition.position);
            Camera.main.transform.position = Vector3.MoveTowards(Camera.main.transform.position, childPosition.position, step);
        }
    
        
        if(Vector3.Distance(Camera.main.transform.position, childPosition.position) < minSnapDistance)
        {
